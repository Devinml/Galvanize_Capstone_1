...
def normal_dist(mean, std):
    # This returns Nomral dist with desired coefs
    return stats.norm(loc=mean, scale=std)


def box_plot_df(df):
    # Filter the data for suspension
...
def fix_aluminum_spelling(string):
    # Fix the Canadian way to spell al.
    if string == 'Aluminium':
        return "Aluminum"
    else:
        return string
...
if __name__ == '__main__':
    # Import Data
    df_29 = pd.read_csv('''/home/devin/Documents/Galvanize/
                        repos/Galvanize_Capstone_1/
                        data/cleaned_data_29.csv''')
    df_275 = pd.read_csv('''/home/devin/Documents/Galvanize/
                        repos/Galvanize_Capstone_1/
                        data/cleaned_data_275.csv''')
    df = pd.read_csv('''/home/devin/Documents/Galvanize/
                    repos/Galvanize_Capstone_1/
    ...
    ax.axvline(mean_29, ymax=norm_29.pdf(mean_29)/(.01775),
               color='#4586AC', ls='--', alpha=.5)
    ax.axvline(mean_275, ymax=norm_275.pdf(mean_275)/(.018),
               color='#C95948', ls='--', alpha=.5)
    plt.text(3190, 0.0143594,
             s=f't-stat = {t_test[0]:.2f}, p-value = 2.38e-38')
    ax.set_xticks([2600,
    ...
    sns.distplot(df_275['Price'], color='#C95948',
                 bins=50, kde=True, ax=ax1, label='27.5')
    sns.distplot(df_29['Price'], color='#4586AC',
                 bins=50, kde=True, ax=ax1, label='29')
    ...
    sns.distplot(df_275['Price'], color='#C95948',
                 bins=bins_, kde=False, ax=ax2, label='27.5')
    sns.distplot(df_29['Price'], color='#4586AC',
                 bins=bins_, kde=False, ax=ax2, label='29')
    ...
    sns.boxplot(x='Front_travel', y='Price', data=box_plot_df(df),
                hue="Wheel_Size", ax=ax3)
